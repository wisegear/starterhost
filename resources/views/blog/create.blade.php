...
<script>
    ...
    @if(isset($post) && $post->images)
        <div id="uploaded-images-preview" style="margin-top: 20px;">
            <h4>Uploaded Images</h4>
            @foreach(json_decode($post->images) as $image)
                <div style="margin-bottom: 10px;">
                    <img src="/{{ $image }}" width="150" style="margin-right: 10px; vertical-align: middle;">
                    <button onclick="copyToClipboard('{{ asset($image) }}')" style="vertical-align: middle;">Click to Copy</button>
                </div>
            @endforeach
        </div>
    @endif
