...
<div class="flex flex-col rounded items-center bg-lime-300 p-4">
    <h2 class="font-bold text-2xl">Link Management</h2>
    <p>Create new link categories and manage existing categories here.</p>
</div>
...
<div>
    <form action="/admin/links" method="post">
        ...
        <div class="flex flex-col border rounded shadow-lg my-10 w-1/2 mx-auto p-6">
            ...
            <label for="new_category_name" class="font-bold mb-2">Enter name of link category</label>
            @error('create_name')
        ...
        <tbody>
            @foreach ($categories as $category)
                <tr>
                    ...
                    <form action="/admin/links/{{ $category->id }}" method="POST" class="flex items-center">
                        @csrf
                        @method('PATCH')
                        ...
                        <td class="text-center">{{ $category->links->count() }}</td>
                    ...
                    <!-- Delete Form -->
                    <form action="/admin/links/{{ $category->id }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <td class="space-x-4 text-center">
                            <button 
                                ...
                                {{ $category->links->count() > 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-500 hover:bg-red-400' }}" 
                                {{ $category->links->count() > 0 ? 'disabled' : '' }}
...
<div>
    <table class="w-full">
        ...
        <tr>
            <td>{{ $link->id }}</td>
            <td><a href="../links/{{ $link->slug }}">{{ $link->title }}</a></td>
            <td>{{ $link->created_at->format('d-m-Y') }}</td>
            <td class="text-center"><a href="../links/{{ $link->id }}/edit"><button class="border rounded bg-lime-500 hover:bg-lime-400 p-1 text-sm">Update</button></a></td>
            <td class="text-center">
                <!-- Delete Form -->
                <form action="../links/{{ $link->id}}" method="POST" onsubmit="return confirm('Are you sure you want to delete this link?');">
                    @csrf
                    @method('DELETE')
